...
def gen_sw_template(
  ...
  return """
    # Move base value into register
    csrr {reg_base}, mngr2proc < {base}
    csrr x2, mngr2proc < {result}
    {nops_base}
    # Instruction under test
    {inst} x2, {offset}({reg_base})
    {nops_dest}
    # Check the result
...
def gen_sw_dest_dep_test( num_nops, inst, base, result ):
  return gen_sw_template( 8, num_nops, "x1", inst, 0, base, result )
...
def gen_sw_base_dep_test( num_nops, inst, base, result ):
  return gen_sw_template( num_nops+3 , 0, "x1", inst, 0, base, result )
...
def gen_ld_base_eq_dest_test( inst, base, result ):
  return gen_ld_template( 3, 0, "x3", inst, 0, base, result )
def gen_sw_base_eq_dest_test( inst, base, result ):
  return gen_sw_template( 3, 0, "x3", inst, 0, base, result )
...
def gen_ld_value_test( inst, offset, base, result ):
  return gen_ld_template( 3, 0, "x1", inst, offset, base, result )
def gen_sw_value_test( inst, offset, base, result ):
  return gen_sw_template( 3, 0, "x1", inst, offset, base, result )
